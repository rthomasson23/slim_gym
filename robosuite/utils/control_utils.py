...
def nullspace_torques(mass_matrix, nullspace_matrix, initial_joint, joint_pos, joint_vel, joint_kp=10):
    ...
    if len(joint_pos) == 9:
        joint_kp_weighted[-3:] = 0.1
        joint_kv_weighted[-3:] = 0.3

    elif len(joint_pos) == 8:
        joint_kp_weighted[-2:] = 0.1
        joint_kv_weighted[-2:] = 0.3

    else:
        joint_kp_weighted[-1:] = 0.1
        joint_kv_weighted[-1:] = 0.3   
